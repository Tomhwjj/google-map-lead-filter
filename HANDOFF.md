...
- **2026-09-09**
  ...
  - **skill_group 友好名统一 + Gmail 分组回填（修「换池后邮箱从 skill 分组消失」误判）**：用户报 skill1 从 50 变 46，怀疑「换池删邮箱是 bug」。查实：换池 `change_pool` 只改 `companies.pool`+`pool_log`、完全不碰 Gmail；真相是那 4 个邮箱在 Gmail 端被手动移到「已回复」组（a.wojda/m.michalak/info@pivit）+ 硬删（mikolaj，404），而本地 `gmail_contacts.skill_group` 列是后来才加的、333 条 synced 全 None，云端变动从不回写→本地↔Gmail 脱节。修复：① `skill_group` 字段语义从「resourceName」改为「友好名」（skill1/已回复/游离），`assign_by_company`/`split_skill_groups` 写回统一存友好名（新增 `_group_resource_by_name` 友好名↔resourceName 转换）；② 新增 `backfill-groups` CLI（`backfill_skill_groups`）——反查每个 synced 联系人 memberships 回填实际分组，404 标 status=invalid（已从 Gmail 删除）；③ 企业详情页邮箱明细加「skill 分组」列。测试 test_company_group.py 断言改友好名 PASS。
  - **gmail_read.py 加 read 命令（读邮件正文）+ 只读授权**：用户要「看到邮件内容判断谁回复了什么」。查证「已回复」组 groupType=USER_CONTACT_GROUP（人工手动建，非 Gmail 自动——Gmail 智能标签只作用于邮件不作用于联系人）。发现项目里**已有** `gmail_read.py`（独立只读 token `data/gmail_read_token.json` + `replies` 查群发回复，最小权限分离设计），一开始误改了 gmail_sync.py 加 readonly scope（重复造轮子），后撤销收敛到正确位置。给 `gmail_read.py` 加 `read` 命令（`--query` Gmail 搜索语法 + `_extract_body` 递归 text/plain 回退 html）。踩坑：① `run_local_server` 的 webbrowser.open 后台进程卡住不弹窗 → 手动方案（临时脚本打印 authorization_url + 本地 http.server 捕获 code，chrome-devtools 打开 URL 完成授权）；② `include_granted_scopes=true` 让 Google 把已授权 contacts/send 合并进返回 scope，与 flow 请求的纯 readonly 冲突报「Scope has changed」→ 去掉该参数。最终 gmail_read_token.json scope 仅 readonly，`read` 验证读正文 PASS（Volt Polska 回复邮件）。用法：`python scripts/gmail_read.py read --query "from:xxx@xx.com" --max 5`。
  - **Volt Polska 背调定性（同行生产商）+ own_brand 甄别规则落地**：读回复邮件发现 Volt Polska 反问「we are producer and importer, you want to sell our products?」——角色错位。背调确认：voltpolska.pl 全自有品牌（Sinus Pro Ultra / ULTRA-5 逆变器储能 / AGM / UPS），brands_found 空、sells_deye=0，是**同行生产商**不是 Deye 渠道。库里它被误判 70 B（产品匹配 0 但渠道 25+规模 25 硬凑，score_basis 写「无逆变器/储能证据」——其实它有产品，只是自有的）。落 task_issues **#12** + qualification-rules.md 加「own_brand 甄别」规则（brands_found 空≠无产品证据，own_brand 渠道分不给、product_tier=own_brand、归黑名单）。⚠️ 纠错：PVhurt(pvhurt.pl) 才是真 Deye 批发商（100 A，brands=Deye/Growatt/Solis，skill1 已联系），与 Volt Polska **不是一家**（只是都用了波兰语 hurt=批发，Volt 的 hurt@ 是批发部门邮箱前缀）。
- **2026-09-07**
...
- **对接文案一律用「派工单」口径，不是「同行会审」（2026-09-22 用户纠正，别回退）**：
  - **WorkBuddy 是我的执行手，不是评审员。** 分工：我负责 **schema / 硬约束 / 共享工具**；**代码他也写**（有编辑权，如 `email_pipeline_wb.py` 由他拥有迭代），**但代码与约束的审查权在我**——schema 变更、写库边界、口径定义，我 review 过才算数。重活（批量读证据判档、回填、重跑、巡检、验收，**及这些活配套的脚本：他写我审**）**归他**。
  - 写 `对接-workbuddy.md` 时必须给：**桶数（实测，不是估计）+ 方法 + 写库路径 + 验收标准 + 优先级 + 阻塞关系**，让他照单执行。
  - **禁止把对接文写成满地「请你复核 / 需你确认」**——那要么是我该拍的板推给了他，要么是把他该干的活降级成了我的提问。
  - 结构上**分开两块**：①**我要他做的**（派工单，带数字与验收）；②**我要他回答我的**（少量，且真会卡我下一步）。别混成一锅「建议 / 期待 / 待确认」。
  - 反例（我犯过）：把 523 家矛盾写成「请你抽查定判例**然后我批量改**」——判断渠道本就是他天天在干的活，批量改也该他做，我只需给脚本支持。
