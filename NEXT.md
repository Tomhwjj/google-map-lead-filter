...
- **市场边界**：只做欧盟 27 国 + 乌克兰（英国/瑞士/挪威/塞尔维亚不做），已写入 SKILL.md / multi-source.md / deye-ecosystem.md
- **双模式评分 + UI 报告规格**（8 要素）已实现：`score_leads.py` / `render_report.py`（四维评分已同步）
- 法国 334 条线索已跑通（数据在 `D:/Agent/tmp/leads_fr_*.json`，**不入 git**）

## 接下来可做（按优先级）

1. **欧盟经销商种子线索落地**：`deye-ecosystem.md` 第六节已列德国/法国/荷兰/西班牙 + 欧洲级的 Deye 官方分销商，可先对这批跑 backfill 补邮箱/电话，作为第一批 A 级存量线索。
2. **扩展到其他目标市场**：目前只做了法国，可跑德国/荷兰/西班牙/意大利/乌克兰等（英国/瑞士/挪威不做）。
